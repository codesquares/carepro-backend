        ...
        public async Task<CaregiverSnapshotResponse> GetSnapshotsAsync(CaregiverSnapshotQuery query)
        {
            ...
            var paged = all
                ...
                .Select(s => new CaregiverJourneySnapshotDTO
                {
                    ...
                    GuarantorCount = s.GuarantorCount ?? 0,
                    ConfirmedGuarantorCount = s.ConfirmedGuarantorCount ?? 0,
                    HasTwoConfirmedGuarantors = s.HasTwoConfirmedGuarantors ?? false,
                    AddressHistoryComplete = s.AddressHistoryComplete ?? false,
                    CaregiverTypeSet = s.CaregiverTypeSet ?? false,
